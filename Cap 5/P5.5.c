...
El programa, al recibir como datos las calificaciones de un grupo de 50 alumnos, obtiene la frecuencia de cada una delas calificaciones y ademas escribe cual es la frecuencia mas alta. */
const int TAM = 50; 
...
void Lectura (int VEC[], int T)
{
  /* La funcion Lectura se utiliza para leer el arreglo de calificaciones. */
  {
    ...
    /* La funcion Impresion se utilizara para imprimir el arreglo de frecuencias. */
    {
      int I;
      for (I = 0; I < T; I++)    
    print("\nVEC[%d]: %d", I, VEC[I]);
    ...
    /* Esta funcion calculala frecuencia de calificaciones. */
    {
      int I; 
      for(I = 0; I < T; I++)
        if ((A[I] >= 0) && (A[I] < 6)) /* Se valida que la calificacion sea correcta. */
     B[A[I]]++; /* Observa la forma de almacenar e incrementar las frecencias. */     
    }
