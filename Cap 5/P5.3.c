...
int main (void)
{
  ...
  *IX = 1;  /* X se modifica, ahora vale 1. */ 
  printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = d% \t Z[0\2] = %d \t Z[3] = % \t Z[4] = %d" , X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
  ...
  *IX = 15;    /* Z[2] se modifica, ahora vale 15. */
  ...
  X = + 5;    /* X se modifica, ahora vale Z[2} + 5 = 20. REcuerde que *IX contiene el valor de Z[2]. */
  ...
  *IX += 1;  /* Z[2] se vuelve a  modificar, ahora vale 12. */ 
  ...
  X = *(IX +1);  /* X Se modifica. El apuntaor IX accede temporalmente a Z[3], por lo tanto X toma el valor de *. Observa que IX no se reasigna */
  Y= *IX;    /* Y se modifica, toma el calor de Z[2] (12) */ 
  printf ( "\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d" , X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
  ...
  IX = IX + 4;  /* IX se modifica. Observa queel apuntador se mueve en 4 posiciones y cae en una direccion afuera del arreglo. 
  Esto ocacionara un error. */ 
  Y = *IX;    /* Y se modifica, toma el valor (basura) de una celda incorrecta. Es un error que no senala el compilador del lenguaje 
C. */ 
  printf ( "\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d" , X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);

  IX = &X;    /* IX apunta a la variable entera de X. */
